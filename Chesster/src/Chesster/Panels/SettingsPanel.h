...
namespace Chesster
{
	...
	{
	public:
		/* Sets all the member variables default values. */
		SettingsPanel();

		/* Draws the Settings panel window to the screen. */
		void OnImGuiRender();
		...
		/* Sets the robot button connection status to Start Server or Stop Server.
		 @param toggle The new true or false state. */
		...
		/* Retrieves the Depth value from the Settings Panel.
		 @return An int of the Depth value being used by the chess engine. */
	...
	private:
		/* This function is called when the Connect/Disconnect Camera button is pressed.
		   It will start the two camera connection threads if there is no connection,
		   or it will disconnect the camera if a connection already exists. */
		void OnCameraButtonPressed();

		/* This function is called when the Start/Stop Server button is pressed.
